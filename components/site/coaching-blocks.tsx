...
export function Pricing({showProcessDetails=false,homeCopy=false}:{showProcessDetails?:boolean;homeCopy?:boolean}={}) {
  return <section id="pricing" className="pricing"><div className="container section">
    <div className="section-heading pricing-heading"><div><p className="eyebrow">코칭 시간과 비용</p><h2>{homeCopy?<>한 달 해보고<br/>결정하세요.</>:<>필요한 만큼 선택하고,<br/>한 달씩 이어갑니다.</>}</h2></div><p>첫 상담에서 하려는 일을 이야기합니다.<br/>시간과 진행 방식이 맞는지 확인한 뒤 결정하세요.</p></div>
    ...
    <div className="price-list">
      ...
      <article className="price-row"><div><p className="kicker">직접 해본 일을 다음 시간에 점검</p><h3>월 코칭</h3></div><div><strong>월 4회 × 2시간 · 총 8시간</strong><p>보통 주 1회 진행합니다. 한 달 단위로 신청하고, 해본 결과와 다음 과제를 보고 연장 여부를 정합니다.</p></div><div className="price-amount"><strong>132<span>만 원</span></strong>{showProcessDetails&&<span className="price-hourly">시간당 16만 5천 원 · 단독 코칭보다 25% 낮은 금액입니다</span>}<span>공급가 120만 원 + 부가세 12만 원</span></div></article>
      <article className="price-row"><div><p className="kicker">필요한 작업을 한 번에 집중해서</p><h3>단독 코칭</h3></div><div><strong>1회 2시간</strong><p>홈페이지 수정이나 데이터 설정 등 필요한 일을 정합니다. 준비된 자료를 보고 두 시간 안에 가능한 작업부터 진행합니다.</p></div><div className="price-amount"><strong>44<span>만 원</span></strong>{showProcessDetails&&<span className="price-hourly">시간당 22만 원</span>}<span>공급가 40만 원 + 부가세 4만 원</span></div></article>
    </div>
