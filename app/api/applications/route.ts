...
export async function POST(request:Request){
 ...
 if(!parsed.success)return reply({error:'사이트 주소를 입력했다면 형식을 확인해 주세요. 상담 내용(10자 이상), AI 사용 경험, 휴대전화번호와 동의는 필수입니다.'},400);
 ...
 await db.prepare('INSERT INTO applications (id, website, problem, ai_usage, phone, consent_version, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(data.id,data.website,data.problem,data.aiUsage,data.phone,'2026-09-12',Date.now()).run();return reply({id:data.id,received:true},201);
