...
export function Method({showSessionExample=false,showProcessLink=true,homeCopy=false}:{showSessionExample?:boolean;showProcessLink?:boolean;homeCopy?:boolean}={}) {
  ...
  return <section id="method" className="container section method-section">
    ...
    <div className="method-sticky"><p className="eyebrow">일대일 코칭 진행 방식</p><h2>{homeCopy?<>보면서 따라<br/>하시면 됩니다.</>:<>실제 작업을 하면서<br/>하나씩 익힙니다.</>}</h2><p className="lead method-lead">한 번에 {MONTHLY_COACHING.hoursPerSession}시간, Zoom으로 화면을 공유합니다. 월 코칭에서는 직접 해본 결과와 어려웠던 부분을 다음 시간에 다시 살펴봅니다.</p>
    ...
    {showSessionExample&&<div className="session-example"><div><p className="eyebrow">홈페이지 제작 기간</p><h3>{formatCourseDuration(COACHING_COURSES.website)}입니다.</h3><p>페이지 수와 기능, 기존 상태와 준비된 자료에 따라 달라집니다.<br/>{homeCopy?`평균 ${COACHING_COURSES.website.months}개월 기준 ${formatManWon(COACHING_COURSES.website.priceWon)} (부가세 포함)입니다. 회차가 늘어나면 월 단위로 이어집니다.`:'첫 데이터 분석까지는 대체로 약 2~3개월입니다.'}</p></div><dl><div><dt>1~2회차</dt><dd>현재 상태와 필요한 페이지를 정하고 제작을 시작합니다</dd></div><div><dt>3~6회차</dt><dd>화면과 내용을 완성하며 SEO·GEO와 측정을 설정합니다</dd></div><div><dt>{`7~${COACHING_COURSES.website.sessions}회차`}</dt><dd>페이지를 보완하고 직접 관리하는 방법을 익힙니다</dd></div><div><dt>측정 후 4주~</dt><dd>GA4·서치콘솔의 첫 데이터를 분석하고 다음 작업을 정합니다</dd></div></dl></div>}
  </section>;
}
...
export function Pricing({showProcessDetails=false,homeCopy=false}:{showProcessDetails?:boolean;homeCopy?:boolean}={}) {
  return <section id="pricing" className="pricing"><div className="container section">
    ...
    <div className="price-list">
      ...
      <article className="price-row"><div><p className="kicker">직접 해본 일을 다음 시간에 점검</p><h3>월 코칭</h3></div><div><strong>{`월 ${MONTHLY_COACHING.sessions}회 × ${MONTHLY_COACHING.hoursPerSession}시간 · 총 ${MONTHLY_HOURS}시간`}</strong><p>보통 주 1회 진행합니다. 한 달 단위로 신청하고, 해본 결과와 다음 과제를 보고 연장 여부를 정합니다.</p></div><div className="price-amount"><strong>{formatManAmount(MONTHLY_COACHING.priceWon)}<span>만 원</span></strong>{showProcessDetails&&<span className="price-hourly">시간당 {formatKoreanWon(REFUND_HOURLY_WON)}</span>}<span>{`공급가 ${formatManWon(MONTHLY_SUPPLY_WON)} + 부가세 ${formatManWon(MONTHLY_VAT_WON)}`}</span></div></article>
    </div>
    <p className="pricing-footnote">모두 부가세 포함 금액입니다. 5주인 달에도 월 코칭은 {MONTHLY_COACHING.sessions}회입니다. 결제는 계좌이체로 진행하며, 규모가 큰 별도 제작은 작업 내용과 비용을 따로 협의합니다.</p>
...
export function PricingSummary() {
  return <section className="container section pricing-summary"><div><p className="eyebrow">코칭 신청 안내</p><h2>첫 상담에서<br/>진행할 일을 정합니다.</h2><p className="lead">1시간 무료 상담 후 코칭을 선택하세요.</p></div><div><dl><div><dt>월 코칭 <span>{`${MONTHLY_COACHING.sessions}회 × ${MONTHLY_COACHING.hoursPerSession}시간`}</span></dt><dd>{formatManWon(MONTHLY_COACHING.priceWon)}</dd></div></dl><p className="meta">부가세 포함 · AI 구독료와 운영 비용은 사용 계정과 항목에 따라 별도 부담</p><a href="/process" className="text-link">진행 방식·포함 내용·비용 자세히 보기 <ArrowRight size={18}/></a></div></section>;
}
